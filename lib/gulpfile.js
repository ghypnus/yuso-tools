...
function dist(done) {
  gulp.src(path.resolve(getProjectPath('dist'), '*.*'), { read: false })
    .pipe(gulpIgnore('*.less'))
    .pipe(gulpRimraf());
  // rimraf.sync();
...
const lintWrapper = cmd => done => {
  ...
  const args = [tslintBin, '-c', tslintConfig, 'src/**/*.tsx'].concat(lastCmd);
...
const tsFiles = ['**/*.tsx', '!node_modules/**/*.*'];
